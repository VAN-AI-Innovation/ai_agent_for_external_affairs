...
- VAN 메인 사이트 톤에 맞춰 프론트엔드 UI를 검정/화이트, 얇은 라인, 운영 콘솔형 레이아웃으로 정리
- 1차 컨택 결과에서 제목/본문이 비어 있거나 내부 지시문이 노출되는 경우를 감지해 재시도 및 fallback 기준 강화
- 추가 맥락에 기업명/기관명을 입력하면 공개 웹 자료를 수집해 AI 입력 맥락에 붙이는 웹 리서치 레이어 추가
- `WEB_RESEARCH_PROVIDER=auto|public_html|gpt` 설정을 추가해 OpenAI API Key 주입 시 GPT-5.6 Luna 기반 웹 리서치 정제 단계로 확장 가능하게 구성
- 계약서 분석 전에 이메일, 전화번호, 주민등록번호, 사업자등록번호, 계좌번호 후보를 마스킹하는 민감정보 처리 레이어 추가
- 계약서 원문과 업로드 파일 bytes를 application log와 DB에 저장하지 않고, request id, 확장자, 파일 크기, 처리 시간 같은 메타데이터만 로깅하도록 변경
- 로컬 개발 포트가 5173 외 포트로 밀려도 CORS 오류가 나지 않도록 개발 환경 CORS 설정 보강
- README에 웹 리서치, GPT-5.6 Luna 전환 지점, 1차 RAG 필터링 + 2차 LLM 검증, 민감문서 보안 처리 선택 이유를 정리

### 선택 이유

- 기업명만 모델에 전달하면 최신성과 출처가 불안정하므로, 백엔드가 먼저 공개 웹 자료와 URL을 수집하고 AI provider가 이를 업무 산출물로 정리하는 구조를 선택
- 나중에 OpenAI API Key를 받으면 기존 업무 로직을 고치지 않고 GPT-5.6 Luna를 웹 리서치 정제와 일반 업무 생성에 연결할 수 있도록 provider 전환 지점을 미리 마련
- 계약서 전체 원문을 LLM이나 로그에 넘기지 않고, 마스킹된 텍스트와 RAG로 좁힌 chunk만 사용해 민감문서 노출 범위를 줄임
- 로컬 템플릿은 모델 실패 시 최후 수단으로만 두고, 1차 컨택처럼 산출물 완성도가 중요한 기능은 출력 검증 기준을 강화

### 확인한 내용

- `..\.venv\Scripts\python.exe -m compileall app` 통과
- `npm run build --prefix frontend` 통과
- `LG` 입력 시 공개 웹 리서치 출처가 결과 맥락에 붙는 것 확인
- 1차 컨택 결과에서 `이메일 제목`, `메일 내용`, `확인 필요 사항`, `참고 출처` 섹션이 생성되는 것 확인
- 마스킹 테스트에서 이메일, 전화번호, 주민등록번호, 사업자등록번호, 계좌번호 후보가 토큰으로 치환되는 것 확인
- 5176 프론트엔드와 8003 백엔드 간 CORS preflight 응답 `200 OK` 확인
